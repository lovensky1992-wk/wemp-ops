...
```bash
# 截图美化
~/.openclaw/workspace/scripts/beautify-screenshot.sh <input> [output] --shadow --bg "#f5f5f5"
```
...
|---------|-----------|-----------|
| "架构"、"分层"、"模型"、"组件"、"系统" | `framework` | notion-sketch / tech-flat |
| "步骤"、"流程"、"先…再…然后"、"第一步" | `flowchart` | notion-sketch / warm-doodle |
| "vs"、"对比"、"优势"、"劣势"、"区别" | `comparison` | tech-flat / notion-sketch |
...
|---|------|------|---------|--------|
| 0 | 封面 | - | 文章主题封面 | 必要 |
| 0.5 | 封面后 | framework | 全文结构总览图 | 推荐 |
| 1 | §2 标题后 | framework | Agent 记忆分层架构 | 必要 |
| 2 | §4 对比段 | comparison | 方案 A vs B 对比 | 推荐 |
| 3 | §6 总结前 | flowchart | 完整工作流总结 | 可选 |
```
...
```bash
# Seedream
~/.openclaw/workspace/scripts/seedream-generate.sh "prompt" output.jpg "2560x1440" 1

# nano-banana-pro (Gemini)
~/.openclaw/workspace/scripts/generate-image.sh "prompt" output.png

# 去水印（nano-banana-pro 生图用）
~/.openclaw/workspace/scripts/remove-watermark.sh <input> [output]
```
